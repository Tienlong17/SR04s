...
def Check_Object(dist):
    ...
    if check_object1 == False: # False nghia la khong co vat can va Tru la co vat can 
        print("thuc hien chuyen dong  lan 1, huong:", dist) # khong co vat can thi duoc di <- se de ham di chuyen 
    else:
...
try:
    while running:
        ...
        pygame.draw.rect(screen,color_button2,(120,350,50,50)) # up 
        pygame.draw.rect(screen,color_button1,(120,430,50,50)) # middle 
        pygame.draw.rect(screen,color_button2,(120,500,50,50)) # down 
        pygame.draw.rect(screen,color_button2,(50,430,50,50)) # left 
        pygame.draw.rect(screen,color_button2,(190,430,50,50)) # right 
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_LEFT: 
                    #pygame.draw.rect(screen,blue_drark,(50,430,50,50))
                    direction = 4
        ...
        if 0 <= direction < 7: # khong truyen so khong thi co di chuyen khong 
            print('direction truoc =',direction)
            Check_Object(direction)
            kiem_tra1 = Call_SR4(1)
            #direction = -1# de chi quet giao dien 1 lan xu ly bien 
            #print('typeMove',typeMove)
            print("ngat")
        # hoặc viết số 0 chạy hàm dừng 
